...
public partial class news_showcats : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["cat_name"] != null)
        {
            lcat.Text = Session["cat_name"].ToString();
            ds = Class1.select("SELECT tbl_news.*, tbl_reporter.fname FROM tbl_news INNER JOIN tbl_reporter ON tbl_news.reporter_id = tbl_reporter.reporter_id where image1 != '" + null + "' AND isbreaking_news = 'YES' AND cat_id = '" + Session["cat_name"].ToString() + "' order by id");
            int k;
            ...
            ds1 = Class1.select("SELECT tbl_news.*, tbl_reporter.fname FROM tbl_news INNER JOIN tbl_reporter ON tbl_news.reporter_id = tbl_reporter.reporter_id where image1 != '" + null + "' AND isbreaking_news = 'NO' AND cat_id = '" + Session["cat_name"].ToString() + "' order by id");
            int k1;
            ...
            ds2 = Class1.select("select * from tbl_news where image1 IS NULL AND cat_id = '" + Session["cat_name"].ToString() + "' order by id ");
            int k2;
